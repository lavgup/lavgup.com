...
export default function Words({
    ...
    return (
        <Container title="Words - Lav" description="Vocables I find intriguing">
            <h1 className="text-4xl font-bold tracking-tight">Words</h1>
            <p className="mt-3">Vocables I find intriguing.</p>
...
function Word({
    ...
    word: InferGetStaticPropsType<typeof getStaticProps>['words'][number];
}) {
    return (
        <div
            key={word.name}
            onClick={() =>
                navigator.clipboard.writeText(word.name.toLowerCase())
            }
        ...
        >
            <div className="flex flex-col ml-2 cursor-copy">
                ...
                <p className="text-xs italic">{word.type}</p>
